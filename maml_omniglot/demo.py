...
def show(args):
    ...
    filename = os.path.join('../output', 'maml_omniglot_'
    ...
    # predict labels
    train_logit = model(support_images_task)
    inner_loss = F.cross_entropy(train_logit, support_labels_task)
    ...
    test_logit = model(query_images_task, params=params)

    with torch.no_grad():
        accuracy += get_accuracy(test_logit, query_labels_task)
